...
################################################################################
	...
	if(EI_CHECK_CONFIG_DIR_WRITABILITY){
	    foreach (WRITABLE_PATH_ARRAY as $checkPath){
	        if(!is_writable($checkPath)){
	            $is_error = true;
	            $error_mg[] = str_replace('_FILE_DIRECTORY_', $checkPath, lang_key('alert_directory_not_writable'));
	...
	if(EI_CHECK_DIRECTORIES_AND_FILES){
		...
		$validations['config_file_dir'] = array(true, 'Config, Logs, Form, Upload directories', $writableCheck , lang_key('writable'), lang_key('no_writable'));
	}
