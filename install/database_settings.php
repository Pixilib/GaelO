...
################################################################################
	...
	if($task == 'send'){
		...
		$database_ssl 		= isset($_POST['database_ssl']) ? $_POST['database_ssl'] : 0;
		...
		if($database_host == ''){
			...
			if(EI_MODE == 'demo'){
				...
				$arr = array();
				$db = Database::GetInstance($database_host, $database_name, $database_username, $database_password, EI_DATABASE_TYPE, false, $database_ssl);
				if($db->Open()){
			...
			if(empty($error_msg)){
				...
				$_SESSION['database_ssl'] = $database_ssl;
		...
		$database_ssl	    = isset($_SESSION['database_ssl']) ? $_SESSION['database_ssl'] : 0;
...
<div id="main">
	...
	<div id="content">
		...
		<div class="central-part">
			...
			<tr>
				<td nowrap>&nbsp;<?php echo lang_key('database_ssl'); ?></td>
				<td>
					<input type="checkbox" class="form_text" name="database_ssl" id="database_ssl" value="1" <?php if(EI_MODE != 'debug') echo 'autocomplete="off"'; ?> onfocus="textboxOnFocus('notes_db_ssl')" onblur="textboxOnBlur('notes_db_ssl')" <?php if($database_ssl) echo("checked")?> />
				</td>
			</tr>
			<tr>
