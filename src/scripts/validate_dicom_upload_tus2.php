...
if ($accessCheck && $role == User::INVESTIGATOR && $visitObject->uploadStatus == Visit::NOT_DONE) {
    ...
    $tempZipRessource = get_tus_file($fileName);
    $tempZipPath = stream_get_meta_data ( $tempZipRessource )['uri'];
    ...
    //A FINIR BOUCLER LES ZIP
    delete_tus_file($zipSize);
...
function delete_tus_file($fileName){
    ...
    $response = $client->request('DELETE', $fileName, ['Tus-Resumable' => '1.0.0']);
