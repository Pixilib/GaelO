...
<script type="text/javascript">
	$(document).ready(function() {
		...
		?> $('input:radio[name="formDecision"]').filter('[value="accepted"]').attr("checked", true);
		<?php
		} else {
		?> $('input:radio[name="formDecision"]').filter('[value="refused"]').attr("checked", true);
		<?php
		}

		if ($visitObject->imageQualityControl == true) {
		?> $('input:radio[name="imageDecision"]').filter('[value="accepted"]').attr("checked", true);
		<?php
		} else {
		?> $('input:radio[name="imageDecision"]').filter('[value="refused"]').attr("checked", true);
		<?php
		}
		?>
		//Add comment data
	...
	$('#refuse, #ask_corrective_action, #accept').click(function(event) {
		...
		if (id == "refuse") {
			...
			if (($("#refused2").is(":checked")) || ($("#refused1").is(":checked"))) {
				...
				if (($("#refused2").is(":checked") && $('#imageComment').val().length === 0) ||
					($("#refused1").is(":checked") && $('#formComment').val().length === 0)) {
					alertifyError('To Refuse quality control, refused item should have associated comment');
				} else {
					check = true;
...
<form method="post" id="controler_form" class="form-horizontal">
	<div id="container_quality_control" class="bloc_bordures">
		...
		<div class="row">
			...
			<div class="col text-center">
				<input type="radio" id="accepted1" name="formDecision" value="accepted">
			</div>
			<div class="col text-center">
				<input type="radio" id="refused1" name="formDecision" value="refused" checked>
			</div>
			<div class="col">
				<input type="text" class="form-control" maxlength="255" id="formComment" name="formComment">
			</div>
		</div>
		<div class="row">
			<div class="col">
			...
			<div class="col text-center">
				<input type="radio" id="accepted2" name="imageDecision" value="accepted">
			</div>
			<div class="col text-center">
				<input type="radio" id="refused2" name="imageDecision" value="refused" checked>
			</div>
			<div class="col">
				<input type="text" class="form-control" maxlength="255" id="imageComment" name="imageComment">
			</div>
		</div>
		<div class="text-center">
			<input type="hidden" value="<?= $patient_num ?>" name="patient_num" id="patient_num"> <input type="hidden" value="<?= $type_visit ?>" name="type_visit" id="type_visit"> <input type="hidden" value="<?= $id_visit ?>" name="id_visit" id="id_visit">
			...
			?>
				<button name="refuse" id="refuse" type="button" class="btn btn-danger">Refuse</button>
				<button name="ask_corrective_action" id="ask_corrective_action" type="button" class="btn btn-secondary">Ask corrective action</button>
				<button name="accept" id="accept" type="button" class="btn btn-success">Accept</button>
			<?php
			}
			?>
