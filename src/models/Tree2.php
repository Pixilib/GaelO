...
class Tree2 {
  ...
  //SK FINIR DE REPRODUIRE LE TREE
  //ET AJOUTER LE NIVEAU VISIT GROUP SI >1GROUPE
  ...
  private function makeInvestigatorTree(){

  }

  private function visitObjectToTreeObject(Visit $visitObject, bool $addClassDecoration){
    ...
    if($addClassDecoration) $jsonVisitLevel['li_attr'] =$attr;
  ...
  public function prepareTree(Study_Visit_Manager $studyVisitManager){
    ...
    if($this->role == User::INVESTIGATOR){
      ...
      foreach($patientObjectArray as $patient){
        ...
        foreach($createdPatientVisits as $createdVisit){
            ...
            $resultTreeArray[] = $this->visitObjectToTreeObject($createdVisit, true);
    ...
    //SK A FAIRE
      ...
      foreach ($dataPatients as $patientCode) {
        ...
        $this->json[] =$this->patientObjectToTreeObject($patientCode);
    ...
    //SK A FAIRE
        ...
        foreach ($createdVisitArray as $visitObject) {
            ...
            $resultTreeArray[] = $this->visitObjectToTreeObject($visitObject, true);
        }
        ...
        foreach ($visitObjectList as $visitObject) {
            ...
            if(  ! in_array($visitObject->patientCode, $patientsArray) ){
                //create a patient entry
                $this->resultTreeArray[] =$this->patientObjectToTreeObject($visitObject->patientCode);
                //SK A FAIRE
