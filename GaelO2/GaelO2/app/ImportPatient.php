...
class ImportPatient extends Model
{
    /**
	...
	public function __construct($originalJson, $study) {
        //Store the JSON file and the target study
        $this->userPrefs=Preference::all();
		$this->originalJson=$originalJson;
		$this->studyObject=new Study($study);
	}
	...
	public function readJson() {
		...
		foreach ($jsonImport as $patient) {
			...
			if ($isNewPatient && $isCorrectPatientNumberLenght && $isPrefixCorrect && $isExistingCenter && !empty($patientRegistrationDate)) {
				...
				if ($this->userPrefs->parseDateImport == 'm.d.Y') {
					$birthDay=intval($birthDateArray[1]);
					$birthMonth=intval($birthDateArray[0]);
				}else if ($this->userPrefs->parseDateImport == 'd.m.Y') {
					$birthDay=intval($birthDateArray[0]);
					$birthMonth=intval($birthDateArray[1]);
				}
				$birthYear=intval($birthDateArray[2]);

				$insertddb=$this->prepareAddPatientToDatabase($patientNumber, $patientLastName, $patientFirstName, $patientGender,
	...
	private function isCorrectPatientNumberLenght($patientNumber) {
		...
		if ($lenghtImport == $this->userPrefs->patientCodeLength) {
			return true;
		}else {
			return false;
		}
    ...
    /**
	 * Prepare to write the patient in the database
	 * @param  $patientNumber
	 ...
	 * @return Array
	 */
	private function prepareAddPatientToDatabase($patientNumber, string $patientLastName, string $patientFirstName, string $patientGender
    ...
    return array('code' => $patientNumber,
