...
class StudyRepository implements StudyRepositoryInterface
{
    ...
    public function addStudy(String $name, string $code, int $patientCodeLength, string $contactEmail, bool $controllerShowAll, bool $monitorShowAll, bool $documentationMandatory, ?string $ancillaryOf, bool $creatablePatientsInvestigator, bool $allowAlphaPatientCode): void
    {
        $study = new Study();
        $study->name = $name;
        $study->code = $code;
        ...
        $study->allow_alpha_patient_code = $allowAlphaPatientCode;
        $study->save();
    }
    ...
    public function getStudyStatistics(string $name, bool $withTrashed): array
    {
        ...
        $study = $withTrashed
            ? $this->studyModel->withTrashed()->findOrFail($name)
            : $this->studyModel->findOrFail($name);

        $dicomStudies = $study->dicomStudies();
        if ($withTrashed) {
